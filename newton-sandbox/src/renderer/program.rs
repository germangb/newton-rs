...
impl Program {
    pub(crate) fn new() -> Program {
        unsafe {
            ...
            Program {
                program,
                world: check!(gl::GetUniformLocation(program, "u_world\0".as_ptr() as *const GLchar)),
                view: check!(gl::GetUniformLocation(program, "u_view\0".as_ptr() as *const GLchar)),
                projection: check!(gl::GetUniformLocation(program, "u_projection\0".as_ptr() as *const GLchar)),
                color: check!(gl::GetUniformLocation(program, "u_color\0".as_ptr() as *const GLchar)),
            }
        }
    }
}
...
unsafe fn create_shader(type_: GLenum, source: &[u8]) -> GLuint {
    ...
    check!(gl::ShaderSource(shader, 1, [source.as_ptr() as *const GLchar].as_ptr() as _, [source.len() as GLint].as_ptr()));
    check!(gl::CompileShader(shader));

    let mut len = 0;
    let mut buffer = vec![0u8; 1024];
    check!(gl::GetShaderInfoLog(shader, buffer.capacity() as GLint, &mut len, buffer.as_mut_ptr() as *mut GLchar));
